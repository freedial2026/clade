...
## 収益要因の徹底検証：利益はすべて「締切オッズで選んだ」ことに由来する (2026-08-14)

これまでに breakeven を超えたと記録された全数字（単勝 ev_best、複勝
ev_best、拡連複 ev_best、walk-forward の 1.6884）について、何が利益を
生んでいたのかを分解した。集計値ではなく **候補1本ごとの明細** を出力し
（`~/analysis/dump_candidates.py` → 1,046,118 行 / 38,766 レース、
`~/analysis/dump_live.py` → 254,181 行 / 1,991 レース、いずれも `.21`）、
同じ行の上で反実仮想規則を再採点することで、モデル・価格帯・枠・尾部・
期間・価格の入手時点をそれぞれ切り分けている。

### 1. 再現性

学習 2023-01-01..2025-07-28 / 検証 2025-07-29..2026-04-17、締切オッズ、
¥100 均等。拡連複は記録値と完全一致（ev_best@1.5 ROI 1.2145 / trimmed
1.1237）。単勝・複勝は記録値よりわずかに上（1.3623/1.2180、
2.0244/1.8548 対 記録の 1.3415/1.1972、1.9854/1.8204）で、差は
`meeting_form` リーク修正（`4d3f328`）を挟んだためと整合する。
**リーク修正で EV 規則の数字はわずかに良くなった**。

### 2. モデルの寄与は本物である（締切価格の上では）

| 統制 | 単勝 @1.5 | 複勝 @1.5 | 拡連複 @1.5 |
|---|---|---|---|
| 実測 ROI | 1.3623 | 2.0244 | 1.2145 |
| 同じオッズ帯を盲目的に買う | 0.6481 | 0.8241 | 0.7247 |
| 同じオッズ帯 **かつ同じ枠/組** を盲目的に買う | 0.8597 | 1.2017 | 0.8303 |
| 帯・枠を統制した後の倍率 | **1.58x** | **1.68x** | **1.46x** |

さらに、どのオッズ帯の内側でも、帯内 EV 四分位が上がるほど平均回収が
単調に上がる（例 複勝 10-20 倍帯：0.19 / 0.57 / 0.98 / 1.73）。
置換検定（各レースの確率ベクトルを別レースのものに差し替え、価格と
払戻はそのまま、40 回）では帰無 ROI は 0.74-0.97 に集中し、40 回中
1.0069 が最大。実測との差は **z = 17.6 〜 56.2**。つまり「価格構造だけ
でそれらしい確率を掛ければ出る数字」ではない。

安定性も高い：複勝 ev_best@1.5 は 10/10 か月・24/24 会場で 1.0 超、
拡連複も 10/10 か月。単勝だけ尾部依存が強く、上位 1% の的中が回収の
**42%** を占める（複勝 19%、拡連複 23%）。特払いの寄与は 0。

### 3. 複勝の「強さ」の一部は下限オッズで期待値を測っていること

`odds_snapshots.place_low`（複勝オッズは範囲提示で、保存しているのは
下限）で EV を計算しているが、実際の払戻は下限の平均 **1.19 倍**、
EV 規則が買う 2-10 倍帯では **1.28-1.37 倍** になる。拡連複も同様に
1.07 倍。単勝は 0.9955（＝提示＝払戻、同着でのみ下振れ）。

したがって「複勝 EV≥1.5」は真の期待払戻でいえば ≈1.9-2.0 の閾値に相当
する。単勝を同じ実効閾値まで上げると ROI 1.5623(@1.8) / 1.6951(@2.0)
で、複勝 2.0244 との差は縮む。**複勝が単勝より優れているという結論は
残るが、記録されていた差の相当部分は閾値の目盛りの違いだった。**

### 4. 決定的：締切前の実価格では、すべて breakeven を割る

2026-08-01..08-13 のライブ捕捉（締切 2 分前の板、完全グリッドのみ、
モデルは 2026-07-31 までで学習）:

| 規則 | 単勝 | 複勝 | 2連単 | 2連複 |
|---|---|---|---|---|
| confidence | 0.9161 | 0.9276 | 0.7012 | 0.7667 |
| ev_best @1.0 | 0.7200 | 0.8090 | 0.4348 | 0.8726 |
| ev_best @1.5 | **0.6501** | **0.7825** | 0.4367 | 0.8690 |

ブートストラップ 95% 区間の上端は単勝 0.848、複勝 0.952 で、いずれも
1.0 に届かない。t-60 / t-10 の板でも同じ（むしろ悪い）。

**これは期間差ではない。** 価格を選択に使わない confidence 規則は
締切データ 0.9015 に対しライブ 0.9161、複勝は 0.9333 対 0.9276 と
ほぼ同じ。変わったのは「選択に使った価格」だけである。

### 5. 機構：提示価格と決済価格の乖離

締切オッズ規則の EV は `p ×（実際の払戻額）` を計算していたに等しい
（単勝で提示/払戻 = 0.9955）。ライブでは選択に使う板と決済価格が別物
になる：

- ライブ t-2 で選ばれた単勝の的中を **提示価格どおり払われたと仮定**
  すると ROI は 1.9514(@1.0) / 2.3922(@1.5)。実際の払戻での ROI は
  0.7200 / 0.6501。**提示の 1/2.7 〜 1/3.7 しか支払われていない。**
- 帯別に見ると、t-2 で 10-20 倍の艇の払戻/提示は 0.65、20-50 倍で
  0.44、50 倍超で 0.17。逆に 1-2 倍は 1.34。
- t-60→t-2 の全艇ドリフトでは、どの帯でも **後に勝つ艇の価格だけが
  縮み、負ける艇は伸びる**（例 5-10 倍帯：勝ち 0.95 倍、負け 2.18 倍）。
  終盤の資金が結果を先取りしており、こちらはその後の価格で払われる。

締切データ側でこれを定量化した感度分析（決済は真の払戻のまま、選択に
使う価格だけを観測されたドリフト幅の**純粋なノイズ**でぼかす。情報付き
資金の相関を含まないので楽観側の上界）:

| | 実測 | ノイズ後 |
|---|---|---|
| 単勝 ev_best@1.5 | 1.3623 | **0.9637** |
| 複勝 ev_best@1.5 | 2.0244 | **1.1576** |
| 拡連複 ev_best@1.5 | 1.2145 | **0.9323** |

純ノイズだけで単勝・拡連複は breakeven を割り、複勝も 1.16 まで落ちる。
実測のライブ（逆選択込み）が 0.65 / 0.78 なのは、この上界と整合する。

### 6. 結論

- モデルには本物の識別力がある（帯・枠統制後 1.46-1.68 倍、置換検定
  z≥17、ライブでも「提示価格が守られれば」1.95-2.39）。
- しかし **記録されてきた収益はすべて、選択に使った価格が決済価格
  そのものだったことに由来する**。パリミュチュエルでは決済価格は
  賭けた後にしか決まらないため、この利益は実行できない。
- 「複勝が単勝より強い」「拡連複が breakeven を超える」は、いずれも
  締切価格の上での話として残るが、実価格での前向き検証はどちらも
  否定的（複勝 0.78）または未実施（拡連複）。
- 拡連複のライブ捕捉は **2026-08-14 20:52 に開始したばかりで 1 レース
  分しかない**（cron の `--with-trifecta` は入っている）。前向き検証は
  数週間後まで不可能。

### 7. 次にやるなら

1. ライブ検証の継続（現状 13 日 / 1,991 レース）。特に拡連複は捕捉が
   始まったばかり。
2. EV の入力を「提示オッズ」から「締切時の払戻の予測」に変える。上の
   ドリフトは系統的（帯依存・単調）なので、板の推移から決済価格を
   推定するモデルが作れる可能性がある。これが唯一、締切価格上の識別力を
   実価格に持ち込む道筋。
3. 複勝の EV は下限オッズではなく上限との中点、または予測払戻で計算し
   直す（現状の閾値は目盛りが 1.28-1.37 倍ずれている）。
### 8. 再現手段（`db/attribute_profit.py`、2026-08-14 に追加）

上の分解はすべて `db/attribute_profit.py` として取り込んだ。候補行列
（レース × 候補）を組み、規則・帯反実仮想・置換検定・尾部・月次・
提示/払戻比・価格ノイズ感度を同じ表の上で計算する。pandas は入れて
いない（ホストに無い、`PROJECT_PROFILE.md` の方針どおり）。

```
python -m boat_prediction.db.attribute_profit \
  --train-start 2023-01-01 --train-end 2025-07-28 \
  --test-start 2025-07-29 --test-end 2026-04-17 \
  --bet-types win,place,wide --market-round closing
```

`--market-round` を `t-60/t-10/t-2` にすると、同じ規則を締切前の実価格
で採点する（ライブ捕捉のうち、その窓の中で最後に見えた気配値を使う）。

`.21` で実行して本文の数字を再現済み。テストは 24 本、うち
`RuleResultsMatchEvaluateBetTypesTest` が `evaluate_bet_types` と
1 ベット単位で一致することを固定している（一致しなければ、この分解は
別の規則を分解していることになる）。全 1,045 テスト green。

**ライブで新たに見えたこと**: 締切価格では帯+枠を統制しても 1.46-1.68 倍
あったモデルの上乗せが、t-2 の実価格では **1.10（単勝）/ 1.06（複勝）**
まで潰れる。実行可能な価格の上では、モデルは価格×枠のセルを盲目的に
買うのとほとんど変わらない。
